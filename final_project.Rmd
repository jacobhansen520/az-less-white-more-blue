...
race2010 <- read_csv("raw-data/ACSDP5Y2010.DP05_data_with_overlays_2020-04-24T142342.csv",
  ...
  select(county, year, hisp_pop, hisp_perc)
...
race <- race2010 %>%
  full_join(race2012, by = c("county", "year", "hisp_pop", "hisp_perc")) %>% 
  full_join(race2014, by = c("county", "year", "hisp_pop", "hisp_perc")) %>% 
  full_join(race2016, by = c("county", "year", "hisp_pop", "hisp_perc")) %>% 
  full_join(race2018, by = c("county", "year", "hisp_pop", "hisp_perc")) %>% 
  arrange(county)
```
...
data %>%
  ...
  mutate(diff_from_2010 = diff_2012 - diff_2010,
         diff_from_2012 = diff_2014 - diff_2012,
         diff_from_2014 = diff_2016 - diff_2014,
         diff_from_2016 = diff_2018 - diff_2016) %>%
  select(county, diff_from_2010, diff_from_2012, diff_from_2014, diff_from_2016) %>% 
  pivot_longer(diff_from_2010:diff_from_2016, names_to = "year", names_prefix = "diff_from_", values_to = "difference") %>% 
  group_by(year) %>% 
  ggplot(aes(x = reorder(county, difference), y = difference, group = year, fill = difference < 0)) +
    geom_col(show.legend = FALSE) +
    theme_classic() +
    labs(x = NULL,
         y = "Registration Gains Relative to Other Party",
         title = "Where Has Each Party Been Improving?",
         subtitle = "Two-Year Periods Between 2010 and 2018") +
    coord_flip() +
